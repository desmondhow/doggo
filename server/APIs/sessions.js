...
const createSessionApiRoute = route =>
    `/:id/sessions/${route}`;
...
router.post(createSessionApiRoute('udc/create-new-session'), function (req, res, next) {
    ...
    if (sessionId) {
        User.findOne({'sessions.UDC.id': sessionId}).exec(function (err, user_ip_obj) {
            if (err) {
                console.log(err)
            } else {
                console.log('Session found', user_ip_obj);
            }
        });
        User.update({'sessions.UDC.id': sessionId}, {$set: {
        ...
        User.findByIdAndUpdate(userId, {$push: {'sessions.UDC': sessionData}}, ((err, updatedUser) => {
            if (err) {
                console.log(err);
...
router.get(createSessionApiRoute('udc/get-current-sessions'), function (req, res) {
    ...
    User.findOne({'_id': req.params.id}).exec(function (err, data) {
        if (err) return res.status(400).send(JSON.stringify({status: false, message: err}));
...
/**
 * Get a specific UDC session by its unique ID
 */
