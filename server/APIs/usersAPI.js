...
router.post("/register", function(req, res) {
  ...
  if (errorsMissingInput) {
    return res
      .status(400)
      .send(JSON.stringify({ message: "Please fill all fields" }));
  }
  ...
  if (req.body.password !== req.body.password_conf) {
    return res
      .status(400)
      .send(JSON.stringify({ message: "Your password does not match!" }));
  } else {
    // Get the values
    const userData = {
    ...
    User.findOne({ email: userData.email }, "name occupation", function(
      err,
      user
    ) {
      if (user !== undefined && user != null) {
        return res
          .status(400)
          .send(JSON.stringify({ message: "You already have an account" }));
      } else {
  ...
  User.findById(req.params.id).exec(function(error, user) {
    if (error) {
      ...
      if (user === null) {
        ...
        return res
          .status(200)
          .send(JSON.stringify({ dogs: user.dogs, handlers: user.handlers }));
      }
    }
  });
});
    ...
    (err, updatedUser) => {
      if (err) {
        ...
        return res
          .status(400)
          .send(
            JSON.stringify({ message: `Error adding trainer to user profile.` })
          );
      }
  ...
  User.findByIdAndUpdate(
    ...
    (err, updatedUser) => {
      if (err) {
        ...
        return res.status(400).send(
          JSON.stringify({
            message: `Error removing trainer from user profile.`
          })
        );
      }
      console.log(`updatedUser: ${JSON.stringify(updatedUser)}`);
    ...
    (err, updatedUser) => {
      if (err) {
        ...
        return res
          .status(400)
          .send(
            JSON.stringify({ message: `Error adding dog to user profile.` })
          );
      }
      console.log(`updatedUser: ${JSON.stringify(updatedUser)}`);
      return res.status(200).send({ message: updatedUser, status: 200 });
    ...
    (err, updatedUser) => {
      if (err) {
        ...
        return res
          .status(400)
          .send(
            JSON.stringify({ message: `Error removing dog from user profile.` })
          );
      }
      console.log(`updatedUser: ${JSON.stringify(updatedUser)}`);
      return res.status(200).send({ message: updatedUser, status: 200 });
