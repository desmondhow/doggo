...
class UDCHidesScreen extends React.Component {
  ...
  _renderHideFields = (concentration, size) => (
    <View style={{flexDirection: 'column', marginLeft: 30, ...center}}>
      <Text h4>{size.toString().replace('#', '.')} cm.</Text>
      <View style={{flexDirection: 'row', marginTop: 15, justifyContent: 'space-between'}}> 
        <View style={{flexDirection: 'column'}}>
        {/* Location */}
          <Text>Location:</Text>
          {renderDropdown(`${concentration}.${size}.Location`, HidesInfo.Locations, styles.dropdown)}
        </View>
        <View style={{flexDirection: 'column'}}>
        {/* Concealed */}
        <Text>Concealed:</Text>
        <View>
            <Field name={`${concentration}[${size}][Concealed]`} component={(inputProps) => {
        ...
        <View style={{flexDirection: 'column'}}>
          ...
          {renderDropdown(`${concentration}[${size}][PlacementArea]`, HidesInfo.PlacementAreas, styles.dropdown)}
        </View>
        {/* Placement Height */}
        <View style={{flexDirection: 'column'}}>
          <Text>Placement Height:</Text>
          {renderDropdown(`${concentration}[${size}][PlacementHeight]`, HidesInfo.PlacementHeights, styles.dropdown)}
        </View>
      </View>
    </View>
  )
      ...
      <ScrollView style={styles.fieldsContainer} keyboardShouldPersistTaps={'handled'}>
        {
          HidesInfo.Hides.map(hideInfo => (
            hideInfo.Concentrations.map(concentration => (
              <View style={{flexDirection: 'column'}}>
                <Text h3 style={{marginLeft: 20}}>{concentration} mil.</Text>
